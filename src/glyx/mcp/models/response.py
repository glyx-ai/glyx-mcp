...
from glyx.mcp.models.cursor import CursorToolCallEvent
...
EVENT_MODEL_MAP: dict[str, type[BaseResponseEvent]] = {
    ...
    ResponseEventType.OUTPUT_TEXT_ANNOTATION_ADDED.value: (
        ResponseOutputTextAnnotationAddedEvent
    ),
    ResponseEventType.FUNCTION_CALL_ARGS_DELTA.value: (
        ResponseFunctionCallArgumentsDeltaEvent
    ),
    ResponseEventType.IMAGE_GENERATION_PARTIAL.value: (
        ResponseImageGenerationPartialEvent
    ),
    ResponseEventType.CODE_INTERPRETER_CODE_DELTA.value: (
        ResponseCodeInterpreterDeltaEvent
    ),
    ResponseEventType.CODE_INTERPRETER_CODE_DONE.value: (
        ResponseCodeInterpreterDoneEvent
    ),
    ResponseEventType.REASONING_SUMMARY_PART_ADDED.value: (
        ResponseReasoningSummaryPartAddedEvent
    ),
    ResponseEventType.REASONING_SUMMARY_PART_DONE.value: (
        ResponseReasoningSummaryPartDoneEvent
    ),
    ResponseEventType.REASONING_SUMMARY_TEXT_DELTA.value: (
...
def _extract_cursor_tool_name(item: Mapping[str, Any]) -> str | None:
    ...
    Cursor-agent events have structure like:
    {"tool_call": {"shellToolCall": {"args": {"command": "ls"}}}}
    {"tool_call": {"readToolCall": {"args": {"path": "file.txt"}}}}
    {"tool_call": {"mcpToolCall": {"serverLabel": "supabase", "name": "execute_sql"}}}
    """
    tool_call = item.get("tool_call")
    if not isinstance(tool_call, Mapping):
        return None
    
    # Tool type is the key inside tool_call (e.g., shellToolCall, readToolCall)
    for tool_type, tool_data in tool_call.items():
        if not isinstance(tool_data, Mapping):
            continue
        
        # For MCP tool calls, use serverLabel + name
        ...
        # For shell tool calls, show the command
        ...
        # For read tool calls, show the path
        ...
        # Generic fallback: convert camelCase to readable name
        # shellToolCall -> shell_tool_call -> shell
        import re
...
def _extract_tool_name(item: Mapping[str, Any]) -> str:
    # First try cursor-agent specific extraction
...
def summarize_tool_activity(
    ...
    # Handle typed cursor-agent tool call events
