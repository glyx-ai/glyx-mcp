...
def create_default_stages() -> list[Stage]:
    ...
    return [
        ...
        Stage(name="Code Review", role=Role.REVIEWER, agent=AgentInstance(base_agent=AgentKey.CLAUDE, role=Role.REVIEWER)),
...
class Pipeline:
    ...
    async def run_stage(self, stage_id: str, prompt: str) -> Artifact | None:
        ...
        try:
            ...
            artifact_type: Literal["code", "review", "test"] = "code" if stage.role == Role.CODER else "review" if stage.role == Role.REVIEWER else "test"
