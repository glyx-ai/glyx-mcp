...
from glyx_sdk.models.cursor import (
    BaseCursorEvent,
...
from glyx_sdk.models.response import (
    BaseResponseEvent,
    parse_response_event,
    summarize_tool_activity,
)
from glyx_sdk.settings import settings
from glyx_sdk.websocket_manager import broadcast_event
...
class ComposableAgent:
    ...
    def from_key(cls, key: AgentKey) -> "ComposableAgent":
        ...
        config_dir = Path(__file__).parent.parent.parent.parent.parent / "agents"
    ...
    def _event_to_event(
        ...
        match event:
            case CursorToolCallEvent():
                return Event(
                    org_id=org_id, org_name=org_name, content=event.tool_name,
                    type="tool_call", actor=self.config.agent_key,
            ...
            case CursorAssistantEvent(message=msg):
                ...
                return Event(
                    org_id=org_id, org_name=org_name, content=content,
                    type="message", actor=self.config.agent_key,
                ) if content else None
            case CursorResultEvent(is_error=is_err, result=result, duration_ms=ms):
                return Event(
                    org_id=org_id, org_name=org_name, content=result,
                    type="error" if is_err else "deployment", actor=self.config.agent_key,
    ...
    async def execute_stream(
        ...
        while True:
            ...
            public_event = {**event, "event": typed_event.model_dump(mode="json")} if isinstance(typed_event, (BaseCursorEvent, BaseResponseEvent)) else event
            yield public_event
