...
async def _orchestrate_and_stream(
    ...
    """Run orchestration and stream EventInsert records back to Linear."""
    try:
        ...
        activities = _fetch_task_events(supabase, task_id, organization_id)
...
def _fetch_task_events(supabase: Any, task_id: str, organization_id: str) -> list[dict[str, Any]]:
    """Fetch activities for a task from Supabase."""
    response = supabase.table("events").select("*").eq("org_id", organization_id).order("created_at", desc=False).execute()
